...
## 第37轮学习 (2026-03-24) - 第85轮迭代

### 新学习的Skills

#### pest-analysis Skill
- PEST分析框架（政治、经济、社会、技术）
- 宏观环境分析标准化模板
- 战略建议输出格式
...
- 竞品对比分析框架
- 业务、财务、估值、成长性对比
- 综合评分系统（业务30%、财务30%、估值20%、成长20%）
- 投资建议输出（最优标的、估值洼地、回避标的）
...
1. **prompts.py 增强**:
   - SYNTHESIZER_PROMPT: 增加PEST分析框架
     - 政治因素 (P)：产业政策、监管环境、贸易政策、地缘政治、政府投资
     - 经济因素 (E)：增长、政策、流动性、汇率、通胀、利率
     - 社会因素 (S)：人口结构、消费趋势、生活方式、环保意识
     - 技术因素 (T)：技术创新、数字化、研发投入、技术替代、专利壁垒
   - SYNTHESIZER_PROMPT: 增加竞品对比分析
     - 对比公司表格（股票代码、名称、市值、主营业务）
     - 业务、财务、估值、成长性对比表格
     - 综合评分表
     - 投资建议（最优标的、估值洼地、回避标的）
   - SYNTHESIZER_PROMPT: 增加事件驱动分析
     - 事件信息（类型、日期、内容）
     - 影响分析（正面、负面、业绩测算）
     - 市场预期分析（事前预期、实际vs预期、预期差）
     - 历史类比和催化剂分析
   - FUNDAMENTAL_ANALYST_PROMPT: 增加价值投资评估
     - 价值投资筛选维度表格
     - 必选条件和加分项
     - 价值投资信号（买入/卖出）
     - 安全边际评估
...
## 第38轮学习 (2026-03-24) - 第86轮迭代

### 新学习的Skills

#### porter-five-forces Skill
- 波特五力分析框架
- 供应商/买方议价能力
- 新进入者/替代品威胁
- 行业内竞争分析
...
1. **prompts.py 增强**:
   - SYNTHESIZER_PROMPT: 增加波特五力分析
     - 供应商/买方议价能力
     - 新进入者/替代品威胁
     - 行业内竞争分析
     - 五力综合评估
   - SYNTHESIZER_PROMPT: 增加BCG矩阵分析
     - 业务定位和象限分析
     - 各象限策略建议
     - 投资优先级和现金流预期
   - RISK_EVALUATOR_PROMPT: 增加风险治理框架
     - 风险治理结构
     - 风险识别方法和评估矩阵
     - 重大风险清单
     - 风险应对策略（规避/转移/降低/接受）
...
#### risk-management (investment) Skill
- 投资风险管理框架
- 风险识别、分类、评估
- 风险控制措施和应急预案
- 风险敞口监控
...
1. **prompts.py 增强**:
   - FUNDAMENTAL_ANALYST_PROMPT: 增加财务报表综合分析
     - 资产负债表结构分析
     - 利润表和费用结构分析
     - 杜邦分析（净利率×资产周转率×权益乘数）
     - 盈利能力、偿债能力、运营能力、成长能力评估
     - 财务健康综合评分（1-5分）

---

## 第40轮学习 (2026-03-24) - 第88轮迭代
...
1. **prompts.py 增强**:
   - SYNTHESIZER_PROMPT: 增强宏观环境分析
     - 增加海外宏观（美国、欧洲）
     - 增加详细经济指标（GDP、PMI、CPI、PPI、LPR）
     - 增加汇率分析和政策预期
     - 增加宏观综合研判
   - SYNTHESIZER_PROMPT: 增加A股市场分析
     - 资金流向表格
     - 市场情绪指标
     - 板块表现分析
     - 技术面分析和综合研判
...
1. **prompts.py 增强**:
   - NEWS_ANALYST_PROMPT: 增加公告解读框架
     - 公告基本信息
     - 核心内容摘要
     - 关键条款提取
     - 历史类比分析
   - NEWS_ANALYST_PROMPT: 增加研报提炼框架
     - 研报基本信息
     - 核心观点和投资逻辑
     - 核心数据提取
     - 研报质量评估
...
1. **prompts.py 增强**:
   - SYNTHESIZER_PROMPT: 增加行业研究报告框架
     - 行业概况和规模
     - 行业发展趋势
     - 行业景气度分析
   - SYNTHESIZER_PROMPT: 增加产业链分析框架
     - 产业链全景图
     - 上中下游各环节分析
     - 价值链分配表格
     - 卡脖子环节识别
...
- 报告结构设计（总分总、故事线、问题解决）
- 幻灯片内容组织
- 图表使用指南（折线图、饼图、柱图、散点图）
- 视觉设计原则（简洁、对比、对齐、留白）

### 应用到Rho项目的改进

1. **prompts.py 增强**:
   - REPORT_GENERATOR_PROMPT: 增加演示设计最佳实践
     - 报告结构设计模板
     - 幻灯片内容组织指南
     - 图表使用指南
     - 视觉设计原则
...
1. **prompts.py 增强**:
   - SENTIMENT_ANALYST_PROMPT: 增强舆情监控报告
     - 增加热度榜单
     - 增强舆情异动分析
     - 优化投资建议结构
...
1. **prompts.py 增强**:
   - REPORT_GENERATOR_PROMPT: 增强图表使用指南
     - 增加可视化代码模板
     - K线图、财务对比图、持仓饼图代码
     - 图表检查清单
...
1. **prompts.py 增强**:
   - SYNTHESIZER_PROMPT: 增加行业特异分析框架
     - 制药行业：管线分析表格、研发能力评估、政策影响
     - 汽车行业：技术路线对比、竞争格局
     - 消费行业：品牌力评估、渠道力评估

---

## 第51轮学习 (2026-03-24) - 第99轮迭代
...
1. **prompts.py 增强**:
   - REPORT_GENERATOR_PROMPT: 增加持续优化框架
     - 反馈收集机制
     - 问题分类统计表格
     - 典型案例分析模板
     - 优化计划和效果追踪
